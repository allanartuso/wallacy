...
export class TestStateService {
  ...
  private readonly resolutionSubject = new BehaviorSubject<ResolutionPayload | null>(null);
  ...
  private readonly runCompleteSubject = new BehaviorSubject<RunCompletePayload | null>(null);
  ...
  private handleMessage(msg: ExtensionMessage): void {
    switch (msg.type) {
      ...
      case "runComplete":
        ...
        if (this.resultsSubject.value.length === 0 && msg.data.results.length > 0) {
